...
local logger = require(".lua.common.logger").new("lrucache.lua")
...
function _M.walk(self)
	for k,v in pairs (self.hasht) do
		logger:info("Key is --> "..tostring(k))
	end
end

function _M.getSize(self)
	return self.size
end
...
function _M.set(self, key, value, ttl)
    ...
    if not node then
        ...
        if queue_is_empty(free_queue) then
            ...
            --         tostring(node))
            logger:notice(key..": evicting oldkey: "..oldkey..", oldnode: "..tostring(node))
            if oldkey then
                hasht[oldkey] = nil
                key2node[oldkey] = nil
            end
